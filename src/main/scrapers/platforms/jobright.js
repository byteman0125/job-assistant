    ...
    try {
      ...
      try {
        ...
        if (finalJobUrl.includes('/apply')) {
          ...
          console.log(`${this.platform}: 🔗 Cleaned URL (removed /apply): ${finalJobUrl}`);
        ...
        // ⚡ FAST PATH: Save immediately without waiting for page load!
        this.updateStatus(`[3/3] 💾 Saving job...`, `Processed: ${totalProcessedCount}`);
        
        const finalCompany = jobCard.company;
        const finalTitle = jobCard.title;
        
        console.log(`${this.platform}: 📝 Saving: "${finalCompany}" - "${finalTitle}"`);
        console.log(`${this.platform}: 🔗 URL: ${finalJobUrl}`);
